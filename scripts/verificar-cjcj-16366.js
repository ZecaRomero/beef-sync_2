...
async function verificarCJCJ16366() {
  try {
    console.log('🔍 Verificando coletas FIV para CJCJ 16366...\n')
    ...
    const coletas = await query(`
      SELECT 
        ...
        cf.created_at
      FROM coleta_fiv cf
      WHERE cf.doadora_nome ILIKE '%CJCJ%16366%'
         OR cf.doadora_id IN (
           SELECT id FROM animais WHERE serie = 'CJCJ' AND rg = '16366'
         )
    ...
    if (coletas.rows.length === 0) {
      console.log('❌ Nenhuma coleta encontrada para CJCJ 16366')
      return
    }
