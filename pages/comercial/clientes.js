...
import { UserGroupIcon, PlusIcon, CurrencyDollarIcon, DocumentTextIcon, CalendarIcon, XMarkIcon, ArrowUpIcon } from '../../components/ui/Icons'
...
export default function Clients() {
  ...
  const loadClientes = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/comercial/clientes-stats')
      if (response.ok) {
  ...
  const loadNotasCliente = async (nomeCliente) => {
    try {
      ...
      if (response.ok) {
        ...
        const notasFiltradas = (data.data || []).filter(nota => 
          nota.tipo === 'saida' && 
          nota.destino && 
          nota.destino.toLowerCase() === nomeCliente.toLowerCase()
        )
        setNotasCliente(notasFiltradas)
      }
    } catch (error) {
  ...
  const handleVerDetalhes = async (cliente) => {
    ...
    await loadNotasCliente(cliente.nome)
  }
  ...
  const clientesFiltrados = clientes.filter(c => 
    ...
    (c.municipio && c.municipio.toLowerCase().includes(busca.toLowerCase()))
  )

  const totalGeral = clientes.reduce((sum, c) => sum + (c.valor_total || 0), 0)
  const totalNotas = clientes.reduce((sum, c) => sum + (c.total_notas || 0), 0)
  ...
  return (
    <div className="space-y-6">
                ...
                {clientesFiltrados.map((cliente, index) => (
                  <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-white">
                      R$ {(cliente.valor_total || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                      {cliente.ultima_venda ? new Date(cliente.ultima_venda).toLocaleDateString('pt-BR') : '-'}
                    </td>
      ...
      {/* Modal de Detalhes */}
      {showDetalhes && clienteSelecionado && (
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <p className="text-xl font-bold text-gray-900 dark:text-white">
                          {clienteSelecionado.total_notas || 0}
                        </p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">
                        ...
                        <p className="text-xl font-bold text-green-600 dark:text-green-400">
                          R$ {(clienteSelecionado.valor_total || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">
                        ...
                        <p className="text-xl font-bold text-gray-900 dark:text-white">
                          {clienteSelecionado.total_itens || 0}
                        </p>
                      </div>
                      {clienteSelecionado.primeira_venda && (
                        <div>
                          <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">
                            Primeira Venda
                          </label>
                          <p className="text-base text-gray-900 dark:text-white">
                            {new Date(clienteSelecionado.primeira_venda).toLocaleDateString('pt-BR')}
                          </p>
                        </div>
                      )}
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                    ...
                    {loadingNotas ? (
                      ...
                      <p className="text-gray-600 dark:text-gray-400">Nenhuma venda encontrada</p>
                    ) : (
                      <div className="overflow-x-auto">
