  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      for (let i = 2; i <= worksheet.rowCount; i++) {
        ...
        const local = row.getCell(3).value?.toString().trim() || '';
        const touroIA = row.getCell(4).value?.toString().trim() || '';
        const seriePai = row.getCell(5).value?.toString().trim() || '';
        const rgPai = row.getCell(6).value?.toString().trim() || '';
        const dataIA = row.getCell(7).value;
        const dataDG = row.getCell(8).value;
        const resultado = row.getCell(9).value?.toString().trim() || '';
