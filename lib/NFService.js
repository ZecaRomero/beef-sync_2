...
class NFService {
  ...
  static async getNotasFiscais(filtros = {}) {
    ...
    if (dataInicio) {
      query += ` AND nf.data_operacao >= $${++paramCount}`;
    ...
    if (fornecedor) {
      query += ` AND fc.nome ILIKE $${++paramCount}`;
  ...
  static async getNaturezasOperacao(tipo = null) {
    let query = 'SELECT * FROM naturezas_operacao WHERE ativo = true';
    const params = [];
  ...
  static async createNaturezaOperacao(dados) {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await pool.query(query, [nome, tipo, descricao || null]);
  ...
  static async updateNaturezaOperacao(id, dados) {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await pool.query(query, [nome, tipo, descricao || null, ativo !== false, id]);
    return result.rows[0];
  }

  // Deletar natureza de operação (soft delete)
  static async deleteNaturezaOperacao(id) {
    const query = 'UPDATE naturezas_operacao SET ativo = false WHERE id = $1';
